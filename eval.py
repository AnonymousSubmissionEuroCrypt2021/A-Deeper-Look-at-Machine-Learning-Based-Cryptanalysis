...
parser.add_argument("--load_data", default=config.compare_classifer.load_data, type=str2bool)
parser.add_argument("--path_to_test", default=config.compare_classifer.path_to_test)

parser.add_argument("--save_data_proba", default=config.compare_classifer.save_data_proba, type=str2bool)

parser.add_argument("--load_model_lgbm", default=config.make_data_classifier.load_model_lgbm, type=str2bool)
parser.add_argument("--load_model_ToT", default=config.make_ToT.load_model_ToT, type=str2bool)
...
try:
    ...
    import pickle
if args.load_model_ToT:
    with open('ToT.p', 'rb') as fp:
        table_of_truth.ToT = pickle.load(fp)
else:
    table_of_truth.create_DDT()
    with open('ToT.p', 'wb') as fp:
        pickle.dump(table_of_truth.ToT, fp, protocol=pickle.HIGHEST_PROTOCOL)
